drop database ebookstoreBinh;
create database if not exists ebookstoreBinh;
use ebookstore;
create table users(
...
create table books(
    ...
    years int,
    genre int,
    status int check (status REGEXP '^[1-3]$'),
    qty int,
    price decimal(10,1),
    createddate date,
    updateddate date,
...
create table customers(
    ...
    member int check (member REGEXP '^[1-4]$'),
...
create table orders(
    ...
    total decimal(10,1),
    orderdate date,
    status int check (status REGEXP '^[0-5]$'),
    createddate date,
    updateddate date,
